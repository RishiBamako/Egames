...
public class CustomRatingBar  extends View {
    ...
    private @ColorInt int fillBorderColor;
    ...
    private Paint paintStarBorderFill;
    ...
    private void initView() {
        ...
        /*paintStarBorderFill = new Paint(Paint.ANTI_ALIAS_FLAG | Paint.DITHER_FLAG);
        paintStarBorderFill.setStyle(Paint.Style.STROKE);
        paintStarBorderFill.setStrokeJoin(Paint.Join.ROUND);
        paintStarBorderFill.setStrokeCap(Paint.Cap.ROUND);
        paintStarBorderFill.setStrokeWidth(starBorderWidth);
        paintStarBorderFill.setPathEffect(cornerPathEffect);*/
    ...
    private void parseAttrs(AttributeSet attrs) {
        ...
        borderColor = arr.getColor(R.styleable.CustomRatingBar_srb_borderColor, getResources().getColor(R.color.golden_stars));
        fillBorderColor = arr.getColor(R.styleable.CustomRatingBar_srb_borderColor, getResources().getColor(R.color.golden_stars));
    ...
    private void validateAttrs() {
        if (numberOfStars <= 0) {
            throw new IllegalArgumentException(String.format("CustomRatingBar initialized with invalid value for numberOfStars. Found %d, but should be greater than 0", numberOfStars));
        }
        if (desiredStarSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE && desiredStarSize
                > maxStarSize) {
            Log.w("CustomRatingBar", String.format("Initialized with conflicting values: starSize is greater than maxStarSize (%f > %f). I will ignore maxStarSize", desiredStarSize, maxStarSize));
        }
        if (stepSize <= 0) {
            throw new IllegalArgumentException(String.format("CustomRatingBar initialized with invalid value for stepSize. Found %f, but should be greater than 0", stepSize));
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.EXACTLY) {
            ...
            if (desiredStarSize != Integer.MAX_VALUE) {
                // user specified ic_account specific star size, so there is ic_account desired width
                ...
                // user specified ic_account max star size, so there is ic_account desired width
    ...
    /**
     * Draws ic_account star in the provided canvas.
     * @param canvas
    ...
    private void drawStar(Canvas canvas, float x, float y, float filled, Gravity gravity) {
        ...
        // Note: below, currentStarSize*0.02f is ic_account minor correction so the user won't see ic_account vertical black line in between the fill and empty color
        if (gravity == Gravity.Left) {
            // color star fill
    ...
    /**
     * Assigns ic_account rating to the touch event.
     * @param x
     * @param y
     */
    ...
    /**
     * Convenience method to convert ic_account value in the given dimension to pixels.
     * @param value
     * @param dimen
     * @return
     */
    ...
    /**
     * Convenience method to convert ic_account value from pixels to the given dimension.
     * @param value
     * @param dimen
     * @return
     */
    ...
    /**
     * Returns ic_account new AnimationBuilder.
     * @return
     */
    ...
    private float normalizeRating(float rating) {
        if (rating < 0) {
            Log.w("CustomRatingBar", String.format("Assigned rating is less than 0 (%f < 0), I will set it to exactly 0", rating));
            return 0;
        } else if (rating > numberOfStars) {
            Log.w("CustomRatingBar", String.format("Assigned rating is greater than numberOfStars (%f > %d), I will set it to exactly numberOfStars", rating, numberOfStars));
            return numberOfStars;
        } else {
            return rating;
        }
    ...
    public interface OnRatingBarChangeListener {
         ...
         * while the user is dragging, only when the user finalizes ic_account rating by
         * lifting the touch.
         *
         * @param CustomRatingBar The RatingBar whose rating has changed.
         * @param rating The current rating. This will be in the range
         *            0..numStars.
         * @param fromUser True if the rating change was initiated by ic_account user'ic_search
    ...
    public class AnimationBuilder {
         ...
         * @param repeatCount must be ic_account positive value or ValueAnimator.INFINITE
         * @return
         */
